 ...
 * @ClassName: SupplierYearReviewNew
 * @Description: 供应商年度评审自动发起类(增加供应商表自定义)
 * @Version: 1.0
 */
public class SupplierYearReviewNew {
    /*
    ...
    */
    ...
    private String SUPPLIER_TABLE = "uf_gysxx";
    ...
    public SupplierYearReviewNew(int company, int year, String supplierTable) {
        this.company = company;
        this.year = year;
        this.SUPPLIER_TABLE = supplierTable;
    }
    ...
    public int createRequests() {
        ...
        String agentQuerySql = String.format(
                "select ifnull(psr,jbr) as jbr,count(1) as supplierCounts,cast((floor(count(1)/%d) +1) as INTEGER) as requestCounts  from %s where ssgs = %d and jbr is not null and gyszt = 0  GROUP BY ifnull(psr,jbr)",
    ...
    private class AgentSupplier {
        ...
        public AgentSupplier setSuppliers(RecordSet recordSet) {
            String querySql = "SELECT id from " + SUPPLIER_TABLE + " where gyszt = 0 and ifnull(psr,jbr) = " + agentId;
        ...
        public int createRequest(RecordSet recordSet) throws Exception {
            ...
            int dept = -1;
            String deptQuerySql = "select departmentid from hrmresource where id = " + agentId;
