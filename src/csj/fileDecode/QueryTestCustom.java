 ...
 * @Version: 1.0
 */
public class QueryTestCustom extends BaseCronJob {

    public static final String BASIC_DEC_PATH = "/home/resource";
    public static final String BASIC_NEW_PATH = "/home/new";

    public static String typePath = "/Zidingyi/";

    public String idsQuerySql =
            "SELECT\n" +
            "\tfd_id \n" +
            "FROM\n" +
            "\tkm_imissive_receive_main \n" +
            "WHERE\n" +
            "\tfd_template_id IN ( SELECT fd_id FROM km_imissive_rectemp WHERE fd_name IN ( '下级单位上报文件（报告）', '下级单位上报文件（请示）' ) ) \n" +
            ...
            "\tkm_review_main \n" +
            "WHERE\n" +
            "\tfd_template_id IN ( SELECT fd_id FROM km_review_template WHERE fd_name IN ( '印章使用审批表', '合同（协议）会签审核单', '印章使用审批表（快速且已有前置流程）', '印章使用审批表（下属企业）' ) ) \n" +
            "\tAND doc_create_time < '2024-08-01' \n" +
            "\tAND auth_area_id = '179936464b7019e946d2f9241b585781' UNION\n" +
            "SELECT\n" +
            "\tfd_id \n" +
            ...
            "WHERE\n" +
            "\tfd_template_id IN ( SELECT fd_id FROM km_imissive_signtemp WHERE fd_name IN ( '部门签报', '下属企业呈报文件', '公司签报', '工作联络函' ) ) \n" +
            "\tAND doc_create_time < '2024-08-01' \n" +
            "\tAND auth_area_id = '179936464b7019e946d2f9241b585781' UNION\n" +
            "SELECT\n" +
            "\tfd_id \n" +
            ...
            "WHERE\n" +
            "\tfd_template_id IN ( SELECT fd_id FROM km_imissive_sendtemp WHERE fd_name IN ( '会议纪要', '工作提示', '公司发文', '党委发文', '工作提示' ) ) \n" +
            "\tAND doc_create_time < '2024-08-01' \n" +
